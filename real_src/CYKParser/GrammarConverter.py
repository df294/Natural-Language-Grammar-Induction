        ...
        if len(rule) == 2 and rule[1][0] != "'":
            ...
            if terminals:
                for item in terminals:
                    ...
                    rule[item[1]] = "{}{}".format(rule[0], str(index))
                    new_rules += ["{}{}".format(rule[0], str(index)), item[0]]
                index += 1
            while len(rule) > 3:
                new_rules += ["{}{}".format(rule[0], str(index)), rule[1], rule[2]]
                rule = [rule[0]] + ["{}{}".format(rule[0], str(index))] + rule[3:]
                index += 1
