...
def generateRandomToken():
    r = random.randint(0, 6 + len(TERMINALS))
    if(r < 7):
        return str(r)
    else:
        return TERMINALS[r - 7]
...
def crossover(parent1, parent2, n):
    ...
    #mix up all the rules between them
    crossMult = random.randint(0,min(int(child2.getSize()/2), int(child1.getSize()/2)))

    saved1 = []
    saved2 = []
    for c in range(crossMult):
        saved1.append(child1.popRandomRule())
        saved2.append(child2.popRandomRule())
...
def mutateChild(child, mutationRate):
    ...
    new_child = child.copySet()
    old_rules = child.getRules()
    for i in range(len(old_rules)):
        key,val = old_rules[i]
        new_child.deleteRule(i)
        res = random.randint(0, 100) / 100.0
        if (res <= mutationRate):
            #mutate
            key = generateRandomNonTerminal()
            if key not in new_child.getNonterminals():
                new_child.addNonterminal(key)

        res = random.randint(0, 100) / 100.0
        if(res <= mutationRate):
            NorT = random.randint(0, 1)
            if NorT == 0:
                new = generateTokenFrom(new_child)
            else:
                new = generateNonTerminalFrom(new_child)
            val = new + val[1]
    ...
    add = random.randint(0, 50) / 100.0
    if (add <= mutationRate):
        addKey = generateRandomNonTerminal()
        NorT = random.randint(0, 1)
        if NorT == 0:
            val1 = generateTokenFrom(new_child)
        else:
            val1 = generateNonTerminalFrom(new_child)
        NorT = random.randint(0, 1)
        if NorT == 0:
            val2 = generateTokenFrom(new_child)
        else:
            val2 = generateNonTerminalFrom(new_child)
...
def createNextPopulation(currentPop):
    ...
    newPop = []
    newPop += crossover(currentPop[0], currentPop[popSize-1], 1)
    newPop += crossover(currentPop[1], currentPop[popSize-2], 1)
    newPop += crossover(currentPop[2], currentPop[popSize-3], 1)
    newPop += crossover(currentPop[3], currentPop[popSize-4], 1)
    parentPool = selectIndividuals(currentPop, int(popSize/6), int(popSize/3)) #TODO: decide on percent of best and lucky
    poolSize = len(parentPool)
    newPop = []
    while (len(newPop) < popSize - 1):
        parent1 = parentPool[random.randint(0, poolSize-1)]
        parent2 = parentPool[random.randint(0, poolSize-1)]
        newPop += crossover(parent1, parent2, 1)
    ...
    return mutateChildrenPool(newPop, MUTATION_RATE) + [currentPop[0]]
